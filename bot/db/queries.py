...
async def get_user_by_username(session: AsyncSession, username: str) -> User | None:
    ...
    на платящем клиенте недопустимо. Из дубликатов берём строку с самым
    свежим `updated_at` (при совпадении — с наибольшим `id`, детерминированный
    тайбрейкер: секундная точность `updated_at`, два `/start` в одну секунду
    иначе дают недетерминированный выбор). Это НЕ «кто писал боту позже
    всех»: `get_or_create_user` трогает `updated_at` только при реальном
    изменении `username`/`full_name` (F10-оптимизация выше по файлу) —
    отдельного `UPDATE` не будет, если профиль не менялся, даже если
    пользователь только что писал боту. Для M-22 (кто-то занял освободившийся
    ник позже) это ровно то, что нужно — переименование в этот ник и есть
    изменение `username`, — но не путать с «был на связи только что».
    """
    # ВАЖНО: strip() и до, и после lstrip("@") — при "  @Ivan" лидирующий
    # пробел блокирует lstrip("@") (тот останавливается на первом же символе
    # не из набора), а при "@ ivan" пробел остаётся МЕЖДУ "@" и ником уже
    # после lstrip — единственного strip() с любой стороны недостаточно.
    needle = username.strip().lstrip("@").strip().lower()
    if not needle:
        return None
